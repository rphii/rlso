...
inline int so_cmp_sp(So *a, So *b) {
    if(!a && !b) return 0;
    if(!a) return so_len(*b);
    if(!b) return so_len(*a);
    return so_cmp_s(*a, *b);
...
inline int so_cmp0_sp(So *a, So *b) {
    if(!a && !b) return 0;
    if(!a) return so_len(*b);
    if(!b) return so_len(*a);

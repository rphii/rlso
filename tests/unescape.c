...
int unescape(So buf, So cmp) {
    So unescaped = SO;
    int result = so_fmt_unescape(&unescaped, buf, SO, SO, 0);
...
int unescape2(So buf, So cmp, char c) {
    So unescaped = SO;
    int result = so_fmt_unescape(&unescaped, buf, so_ll(&c, 1), SO, 0);
    printff("COMPARE: (result %u)",result);
    so_printdbg(unescaped);
    so_printdbg(cmp);
    if(!result) result = so_cmp(unescaped, cmp);
...
int unescape_fail(So buf, size_t i) {
    int result = 0;
    So unescaped = SO;
    size_t i_err = so_fmt_unescape(&unescaped, buf, SO, SO, 0);
...
int main(void) {
    ...
    So tmp = SO;
    so_clear(&tmp);
    result |= so_fmt_unescape(&tmp, so("'"), so("'"), SO, 0);
    so_printdbg(tmp);
    EXPECT_CMP(tmp, so(""));

    so_clear(&tmp);
    result |= so_fmt_unescape(&tmp, so("a'"), so("\'"), SO, 0);
    so_printdbg(tmp);
    EXPECT_CMP(tmp, so("a"));

    so_clear(&tmp);
    result |= so_fmt_unescape(&tmp, so("asdf]lol"), so("]"), SO, 0);
    so_printdbg(tmp);
    EXPECT_CMP(tmp, so("asdf"));

    so_clear(&tmp);
    result |= (so_fmt_unescape(&tmp, so("[asdf["), so("]"), SO, 0) != -1);
    so_printdbg(tmp);
    EXPECT_CMP(tmp, so(""));
